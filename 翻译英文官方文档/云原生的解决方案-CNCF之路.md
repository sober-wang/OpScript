...
Cloud Native 基金会归属于 Linux 基金会名下，用于构建和管理新一代应用程序开发平台和解决方案。它是为了新一代应用程序开发组建的基金会。CNCF 将云原生定义为在“新一代动态环境”中运行的“可扩展应用程序”，这些应用程序使用容器，微服务和 API 声明等技术。 Kubernetes 是世界上最受欢迎的容器编排平台，也是 CNCF 的第一个项目。
